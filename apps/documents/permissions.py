...
def can_view_document(user: "User", document: "Document") -> bool:
    ...
    if document.confidentiality_level in (
        ...
        if not _check(user, DocumentPermissions.VIEW_CONFIDENTIAL):
            return False
    return True
...
def can_upload_version(user: "User", document: "Document") -> bool:
    if not _check(user, DocumentPermissions.UPLOAD_VERSION):
        return False
    return document.owner_id == user.id or _check(user, DocumentPermissions.UPLOAD_VERSION)


def can_checkout(user: "User", document: "Document") -> bool:
    if not _check(user, DocumentPermissions.CHECKOUT):
        return False
    return True


def can_checkin(user: "User", document: "Document") -> bool:
    if not _check(user, DocumentPermissions.CHECKIN):
        return False
    return True


def can_cancel_checkout(user: "User", document: "Document") -> bool:
    if not _check(user, DocumentPermissions.CANCEL_CHECKOUT):
        return False
    return True
