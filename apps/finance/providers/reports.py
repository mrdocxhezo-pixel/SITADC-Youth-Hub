...
class FinanceReportsProvider:
    ...
    def get_income_statement(
        ...
        income_by_source = income_txns.values('source').annotate(
            total=Sum('amount')
        ).order_by('-total')
        
        expense_by_category = expense_txns.values('source').annotate(
            total=Sum('amount')
        ).order_by('-total')
        
        total_income = income_txns.aggregate(total=Sum('amount'))['total'] or Decimal('0')
        total_expenses = expense_txns.aggregate(total=Sum('amount'))['total'] or Decimal('0')
    ...
    def get_balance_sheet(
        self, 
        as_of_date: Optional[timezone.datetime] = None
    ) -> Dict[str, Any]:
        """
        ...
        for account in accounts:
            balance = account.get_balance_as_of_date(as_of_date)  # Assuming this method exists
            
            account_data = {
                'id': account.id,
                'name': account.name,
                'code': getattr(account, 'account_code', ''),
                'balance': balance,
            ...
            elif account.account_type in ['equity', 'income']:
        ...
        if equity:
            ...
            if total_equity != 0:
                for eq_account in equity:
                    proportion = eq_account['balance'] / total_equity if total_equity > 0 else 0
                    eq_account['balance'] += equity_adjustment * proportion
            else:
                # If no equity accounts, put all adjustment in first equity account or create one
                if equity:
    ...
    def get_cash_flow_statement(
        ...
        operating_income = get_accessible_transactions(self.user).filter(
            ...
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            source__in=['donation', 'grant', 'sponsorship', 'membership', 'service_fee']
        ...
        operating_expenses = get_accessible_transactions(self.user).filter(
            ...
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            source__in=['program_expenses', 'administrative', 'salaries', 'utilities', 'rent']
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0')
        ...
        investing_inflows = get_accessible_transactions(self.user).filter(
            ...
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            source__in=['asset_sale', 'investment_return']
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0')
        ...
        investing_outflows = get_accessible_transactions(self.user).filter(
            ...
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            source__in=['asset_purchase', 'investment', 'capital_expenditure']
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0')
        
        net_investing_cash_flow = investing_inflows - investing_outflows
        
        # Financing activities: loans, donations, equity
        financing_inflows = get_accessible_transactions(self.user).filter(
            transaction_type='INCOME',
            status='POSTED',
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            source__in=['loan_proceeds', 'equity_contribution', 'debt_financing']
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0')
        ...
        financing_outflows = get_accessible_transactions(self.user).filter(
            ...
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            source__in=['loan_repayment', 'dividend_payment', 'debt_financing']
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0')
        ...
        # Beginning and ending cash (simplified - would come from cash accounts in real system)
        beginning_cash = Decimal('0')  # Would get from cash account balance at start_date
    ...
    def get_grant_report(
        ...
        Args:
            grant_id: ID of specific grant (optional). If None, returns summary of all grants.
        ...
        if grant_id:
            ...
            try:
                ...
                if not accessible_grants.filter(id=grant_id).exists():
                    raise PermissionError("You do not have permission to access this grant.")
    ...
    def get_donor_report(
        ...
        Args:
            donor_id: ID of specific donor (optional). If None, returns summary of all donors.
        ...
        if budget_id:
            ...
            return {
                'summary': {
                    ...
                    'total_remaining': total_remaining,
                    'overall_variance_percentage': ((total_budgeted - total_allocated) / total_budgeted * 100) if total_budgeted > 0 else Decimal('0'),
                },
                'by_financial_year': [
                    {
