...
from apps.finance.selectors import get_accessible_budgets
...
class BudgetingProvider:
    ...
    def get_budget_summary(self) -> dict[str, Any]:
        ...
        utilization_by_year = (
            budgets.values("financial_year__name")
            .annotate(
        ...
        return {
            "summary": {
                ...
                "utilization_percentage": (
                    (total_allocated / total_budgeted * 100)
                    if total_budgeted > 0
                    else Decimal("0")
                ),
            ...
            "by_financial_year": [
                {
                    ...
                    "utilization_percentage": (
                        (item["total_allocated"] / item["total_budgeted"] * 100)
                        if item["total_budgeted"] > 0
                        else Decimal("0")
                    ),
            ...
            "recent_budgets": [
                {
                    ...
                    "code": budget.code,
                    "financial_year": (
                        budget.financial_year.name if budget.financial_year else None
                    ),
    ...
    def get_budget_variance_report(
        ...
        if budget_id:
            ...
            try:
                ...
                # (BudgetAllocation model lookup omitted - not needed here)

                # Get transactions against this budget if available
                # (Transaction model lookup omitted - not needed here)
                        ...
                        "status": (
                            ...
                            if variance_pct >= -5
                                ...
                                if variance_pct >= -15
                                    ...
                                    "underutilized" if variance_pct < -15 else "overrun"
                                )
                            )  # Caught by the budget service in practice
                        ),
                    }
                )
            ...
            # Categorize budgets
            on_track_count = len(
                [b for b in budget_details if b["status"] == "on_track"]
            )
            caution_count = len([b for b in budget_details if b["status"] == "caution"])
            underutilized_count = len(
                [b for b in budget_details if b["status"] == "underutilized"]
            )
    ...
    def get_budget_allocations_report(
        ...
        if budget_id:
            ...
            try:
                ...
                total_allocated = allocations.aggregate(total=Sum("allocated_amount"))[
                    "total"
                ] or Decimal("0")
                total_spent = allocations.aggregate(total=Sum("spent_amount"))[
                    "total"
                ] or Decimal("0")
                total_variance = total_allocated - total_spent
                    ...
                    "allocations": [
                        {
                            ...
                            "variance": alloc.allocated_amount - alloc.spent_amount,
                            "variance_percentage": (
                                (
                                    (alloc.allocated_amount - alloc.spent_amount)
            ...
            # This would be more complex and might be better served by
            # specific budget reports
            return {
                "note": (
                    "For overall budget allocations, please run reports "
    ...
    def get_budget_forecast(
        self, budget_id: int, months_ahead: int = 3
    ) -> dict[str, Any]:
        """
        Get budget forecast.
        ...
        monthly_forecast = (
            remaining_budget / months_ahead if months_ahead > 0 else Decimal("0")
        )

        forecast_periods = []
        cumulative_spent = (
            budget.allocated_amount - remaining_budget
        ...
        for i in range(1, months_ahead + 1):
            ...
            forecast_periods.append(
                {
                    "period": (
                        f"{period_start.strftime('%Y-%m')} to "
                    ...
                    "remaining_budget": max(
                        remaining_budget - (monthly_forecast * i), Decimal("0")
                    ),
                }
            )
