...
class SponsorsProvider:
    ...
    def get_sponsors_summary(self) -> Dict[str, Any]:
        ...
        average_contribution = sponsors.aggregate(avg=Avg('total_contributions'))['avg'] or Decimal('0')
        
        # Sponsors by type (if sponsor_type field exists)
        if hasattr(sponsors.first() if sponsors.exists() else None, 'sponsor_type'):
            by_type = sponsors.values('sponsor_type').annotate(
                count=Count('id'),
        ...
        top_sponsors = sponsors.order_by('-total_contributions')[:10]
        
        # Recently active sponsors (sponsored in last 90 days)
        ninety_days_ago = timezone.now() - timezone.timedelta(days=90)
        # This would require linking sponsorships to transactions - assuming we have a way to track this
        # For now, we'll use a placeholder based on when sponsor records were updated
        recently_active = sponsors.filter(
            ...
            'by_type': [
                {
                    ...
                    'average_contribution': item['total_contributions'] / item['count'] if item['count'] > 0 else Decimal('0'),
                }
                for item in by_type
            ] if by_type else [],
            'top_sponsors': [
                {
                    ...
                    'total_contributions': sponsor.total_contributions,
    ...
    def get_sponsorship_trends(
        ...
        # For now, we'll return a placeholder structure based on sponsor creation/update dates
        ...
        for sponsor in sponsors:
            ...
            yearly_data[year]['total_contributions'] += sponsor.total_contributions
            ...
            'data': [
                {
                    ...
                    'average_contribution': data['total_contributions'] / data['count'] if data['count'] > 0 else Decimal('0'),
                }
                for year, data in sorted_data
            ],
            'summary': {
                'total_new_sponsors': sum(data['count'] for data in yearly_data.values()),
                'total_contributions': sum(data['total_contributions'] for data in yearly_data.values()),
            },
        }
    ...
    def get_sponsorship_benefits_analysis(self) -> Dict[str, Any]:
        ...
        """
        # This would require tracking what sponsors receive in return for their sponsorship
        # For now, we'll return a placeholder structure
        return {
            'note': 'Sponsorship benefits analysis requires tracking of sponsorship agreements and deliverables.',
