...
class FinanceAnalyticsProvider:
    ...
    def get_income_trends(
        self, months: int = 12, group_by: str = "month"
    ) -> dict[str, Any]:
        """
        ...
        if group_by == "month":
            ...
            return {
                ...
                "average_monthly_income": (
                    sum(data["income"] for data in trends.values()) / len(trends)
                    if trends
                    else Decimal("0")
                ),
            }
    ...
    def get_expense_trends(
        self, months: int = 12, group_by: str = "month", by_source: bool = False
    ) -> dict[str, Any]:
        """
        ...
        if by_source:
            ...
            trends = defaultdict(
                lambda: defaultdict(lambda: {"expense": Decimal("0"), "count": 0})
            )
            ...
            return {
                ...
                "average_period_expense": (
                    sum(data["expense"] for data in trends.values()) / len(trends)
                    if trends
                    else Decimal("0")
                ),
            }
    ...
    def get_budget_variance_analysis(self) -> dict[str, Any]:
        ...
        for budget in budgets:
            ...
            if (
                variance_pct >= -5
            ):  # Within 5% of budget (underspent or overspent by less than 5%)
                on_track += 1
            elif variance_pct >= -15:  # Between 5% and 15% underspent
                caution += 1
            elif variance_pct < -15:  # More than 15% underspent
                underutilized += 1
            # Note: We're not categorizing overspending here as it's handled
        ...
        # Calculate ratios
        budget_utilization = (
            (total_actual / total_budgeted * 100)
            if total_budgeted > 0
            else Decimal("0")
        )
    ...
    def get_funding_source_analysis(self) -> dict[str, Any]:
        ...
        grant_analysis = {
            "total_awarded": grants.aggregate(total=Sum("amount_awarded"))["total"]
            or Decimal("0"),
            "total_disbursed": grants.aggregate(total=Sum("disbursed_amount"))["total"]
            or Decimal("0"),
            "average_grant_size": grants.aggregate(avg=Avg("amount_awarded"))["avg"]
            or Decimal("0"),
            "count": grants.count(),
            "by_status": list(
                grants.values("status")
                .annotate(count=Count("id"), total=Sum("amount_awarded"))
                .order_by("-total")
            ),
            "by_donor_type": list(
                grants.values("donor__donor_type")
                .annotate(count=Count("id"), total=Sum("amount_awarded"))
                .order_by("-total")
                if grants.exists()
                else []
        ...
        donor_analysis = {
            "total_contributions": donors.aggregate(total=Sum("total_donated"))["total"]
            or Decimal("0"),
            "average_donation": donors.aggregate(avg=Avg("total_donated"))["avg"]
            or Decimal("0"),
            "count": donors.count(),
            "by_type": list(
                donors.values("donor_type")
                .annotate(count=Count("id"), total=Sum("total_donated"))
                .order_by("-total")
            ),
            "top_donors": list(
                donors.order_by("-total_donated")[:10].values(
                    "name", "donor_type", "total_donated"
                )
            ),
        }
        ...
        sponsor_analysis = {
            "total_contributions": sponsors.aggregate(total=Sum("sponsored_amount"))[
                "total"
            ]
            or Decimal("0"),
            "average_sponsorship": sponsors.aggregate(avg=Avg("sponsored_amount"))[
                "avg"
            ]
            or Decimal("0"),
            ...
            "by_type": list(
                ...
                .annotate(count=Count("id"), total=Sum("sponsored_amount"))
                .order_by("-total")
                if hasattr(sponsors.first(), "sponsor_type")
                else []
            ),
            "top_sponsors": (
                list(
                    sponsors.order_by("-sponsored_amount")[:10].values(
                        "name", "sponsor_type", "sponsored_amount"
                    )
                )
                if hasattr(sponsors.first(), "sponsor_type")
                else []
        ...
        campaign_analysis = {
            "total_pledged": campaigns.aggregate(total=Sum("target_amount"))["total"]
            or Decimal("0"),
            "total_raised": campaigns.aggregate(total=Sum("amount_raised"))["total"]
            or Decimal("0"),
            "average_target": campaigns.aggregate(avg=Avg("target_amount"))["avg"]
            or Decimal("0"),
        ...
        if sponsor_analysis["total_contributions"] > 0:
            funding_sources.append(
                ("sponsors", sponsor_analysis["total_contributions"])
            )
        ...
        for source_type, amount in funding_sources:
            percentage = (
                (amount / total_funding * 100) if total_funding > 0 else Decimal("0")
            )
    ...
    def get_financial_ratios(self) -> dict[str, Any]:
        ...
        return {
            ...
            "efficiency_ratios": {
                ...
                "funding_efficiency": None,  # Would require funds raised/funds
                # requested
            },
            "solvency_ratios": {
            ...
            "note": (
                "Financial ratios require integration with balance sheet "
    ...
    def get_cash_flow_analysis(self) -> dict[str, Any]:
        ...
        return {
            ...
            "note": (
                "Detailed cash flow analysis requires proper transaction "
                "categorization and opening/closing cash balances."
            ),
        }
