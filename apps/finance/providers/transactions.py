...
from apps.finance.selectors import get_accessible_transactions
...
class TransactionsProvider:
    ...
    def get_transaction_summary(
        ...
        # By source
        by_source = transactions.values('source').annotate(
            total=Sum('amount'),
            count=Count('id')
        ).order_by('-total')
        ...
        return [
            {
                ...
                'financial_account': txn.financial_account.name if txn.financial_account else None,
    ...
    def get_transactions_by_account(
        ...
        if not accessible_accounts.filter(id=account_id).exists():
            raise PermissionError("You do not have permission to access this financial account.")
        ...
        for txn in transactions:
            ...
            if hasattr(txn, 'financial_account') and txn.financial_account_id == account_id:
        ...
        return [
            {
                ...
                'amount': txn.amount,
                'currency': txn.currency,
                'description': txn.description[:100] + ('...' if len(txn.description) > 100 else ''),
                'financial_account': txn.financial_account.name if txn.financial_account else None,
                'budget': txn.budget.name if txn.budget else None,
            }
            for txn in transactions
    ...
    def get_transaction_trends(
        ...
        return {
            ...
            'average_monthly_amount': sum(data['amount'] for data in monthly_data.values()) / len(monthly_data) if monthly_data else Decimal('0'),
        }