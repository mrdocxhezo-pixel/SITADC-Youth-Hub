...
class DonorsProvider:
    ...
    def get_donors_summary(self) -> Dict[str, Any]:
        ...
        average_contribution = donors.aggregate(avg=Avg('total_contributions'))['avg'] or Decimal('0')
        ...
        top_donors = donors.order_by('-total_contributions')[:10]
        
        # Recently active donors (donated in last 90 days)
        ninety_days_ago = timezone.now() - timezone.timedelta(days=90)
        # This would require linking donations to transactions - assuming we have a way to track this
        # For now, we'll use a placeholder based on when donor records were updated
        recently_active = donors.filter(
    ...
    def get_donor_giving_trends(
        ...
        # For now, we'll return a placeholder structure based on donor creation/update dates
        ...
        for donor in donors:
            ...
            yearly_data[year]['total_contributions'] += donor.total_contributions
    ...
    def get_donor_retention_analysis(self) -> Dict[str, Any]:
        ...
        return {
            'note': 'Donor retention analysis requires tracking of donation history over time.',
            ...
            'reactivation_rate': None,  # Percentage of lapsed donors who start giving again
            ...
            'by_type': [
                {
                    'type': item['donor_type'],
                    'count': item['count'],
                    'percentage': (item['count'] / donors.count() * 100) if donors.count() > 0 else Decimal('0'),
                }
                for item in by_type
            ],
    ...
    def get_large_donors(
        ...
        """
        donors = get_accessible_donors(self.user).filter(
            total_contributions__gte=threshold_amount
        )
        ...
        return [
            ...
            for donor in donors.order_by('-total_contributions')
        ]