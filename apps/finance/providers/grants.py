...
from apps.finance.selectors import get_accessible_donors, get_accessible_grants
...
class GrantsProvider:
    ...
    def get_grants_summary(self) -> dict[str, Any]:
        ...
        # Average grant size
        average_grant = grants.aggregate(avg=Avg("amount_awarded"))["avg"] or Decimal(
            "0"
        )
        ...
        # Grants by donor
        by_donor = (
            grants.values("donor__name", "donor__id")
            .annotate(
                count=Count("id"),
                total_amount=Sum("amount_awarded"),
                total_disbursed=Sum("disbursed_amount"),
        ...
        recent_awards = grants.filter(status__in=["AWARDED", "ACTIVE"]).order_by(
            "-award_date"
        )[:5]
        ...
        return {
            "summary": {
                ...
                "disbursement_rate": (
                    (total_disbursed / total_awarded * 100)
                    if total_awarded > 0
                    else Decimal("0")
                ),
            },
            "by_status": [
                {
                    ...
                    "disbursement_rate": (
                        (item["total_disbursed"] / item["total_amount"] * 100)
            ...
            "ending_soon": [
                {
                    ...
                    "amount": grant.amount_awarded,
                    "end_date": grant.end_date,
                    "days_remaining": (grant.end_date - timezone.now().date()).days,
                }
                for grant in ending_soon
            ],
        }

    def get_grant_funding_trends(self, years: int = 5) -> dict[str, Any]:
        """
        ...
        grants = get_accessible_grants(self.user).filter(
            award_date__gte=start_date, award_date__lte=end_date
        ...
        for grant in grants:
            year = (
                grant.award_date.year
                if hasattr(grant, "award_date")
                else grant.created_at.year
            )
            yearly_data[year]["count"] += 1
            yearly_data[year]["total_awarded"] += grant.amount_awarded
        ...
        return {
            ...
            "data": [
                {
                    ...
                    "average_grant_size": (
                        data["total_awarded"] / data["count"]
                        if data["count"] > 0
                        else Decimal("0")
                    ),
                    "disbursement_rate": (
                        (data["total_disbursed"] / data["total_awarded"] * 100)
            ...
            "summary": {
                ...
                "total_awarded": sum(
                    data["total_awarded"] for data in yearly_data.values()
                ),
                "total_disbursed": sum(
                    data["total_disbursed"] for data in yearly_data.values()
                ),
                "total_remaining": sum(
                    data["total_awarded"] - data["total_disbursed"]
                    for data in yearly_data.values()
                ),
            },
        }

    def get_donor_grant_analysis(self, donor_id: int | None = None) -> dict[str, Any]:
        """
        Get grant analysis for a specific donor or all donors.

        Args:
            donor_id: ID of specific donor (optional). If None, returns analysis
            for all donors.
        ...
        if donor_id:
            ...
            try:
                ...
                grants = grants.filter(
                    id__in=accessible_grants.values_list("id", flat=True)
                )
                ...
                return {
                    ...
                    "grants": {
                        ...
                        "total_disbursed": total_disbursed,
                        "total_remaining": total_remaining,
                        "average_grant_size": grants.aggregate(
                            avg=Avg("amount_awarded")
                        )["avg"]
                        or Decimal("0"),
                    },
                    "by_status": [
            ...
            return {
                ...
                "summary": {
                    ...
                    "total_disbursed": sum(
                        d["total_disbursed"] for d in donor_analysis
                    ),
                    "total_remaining": sum(
                        d["total_remaining"] for d in donor_analysis
                    ),
                },
            }
    ...
    def get_grant_probability_analysis(self) -> dict[str, Any]:
        ...
        return {
            "note": (
                "Grant probability analysis requires tracking of grant "
