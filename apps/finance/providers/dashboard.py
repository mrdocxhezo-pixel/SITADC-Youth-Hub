...
class FinanceDashboardProvider:
    ...
    def get_financial_summary(self) -> Dict[str, Any]:
        ...
        for account in accounts:
            balance = account.get_current_balance()  # Assuming this method exists on the model
    ...
    def get_budget_overview(self) -> Dict[str, Any]:
        ...
        for budget in budgets:
            ...
            budget_details.append({
                ...
                'variance_percentage': variance_data.get('variance_percentage', Decimal('0')),
                'status': 'on_track' if variance_data.get('variance_percentage', Decimal('0')) >= -5 else 'overrun' if variance_data.get('variance_percentage', Decimal('0')) < -10 else 'caution',
            })
    ...
    def get_funding_overview(self) -> Dict[str, Any]:
        ...
        total_granted = grants.aggregate(total=Sum('amount'))['total'] or Decimal('0')
        ...
        total_donations = donors.aggregate(total=Sum('total_contributions'))['total'] or Decimal('0')
        
        # Get sponsors
        sponsors = get_accessible_sponsors(self.user)
        total_sponsorship = sponsors.aggregate(total=Sum('total_contributions'))['total'] or Decimal('0')
    ...
    def get_recent_transactions(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        transactions = get_accessible_transactions(self.user).order_by('-transaction_date')[:limit]
        ...
        for transaction in transactions:
            transaction_list.append({
                ...
                'description': transaction.description[:100] + ('...' if len(transaction.description) > 100 else ''),
            })
    ...
    def get_budget_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'overrun': overrun,
            'total': total,
            'on_track_percentage': (on_track / total * 100) if total > 0 else Decimal('0'),
        }
    ...
    def get_funding_sources(self) -> List[Dict[str, Any]]:
        ...
        # Grants
        grants = get_accessible_grants(self.user)
        grant_total = grants.aggregate(total=Sum('amount'))['total'] or Decimal('0')
        if grant_total > 0:
            sources.append({
                'type': 'grant',
                'name': 'Grants',
        ...
        # Donations
        donors = get_accessible_donors(self.user)
        donation_total = donors.aggregate(total=Sum('total_contributions'))['total'] or Decimal('0')
        if donation_total > 0:
            sources.append({
                'type': 'donation',
                'name': 'Donations',
        ...
        # Sponsorships
        sponsors = get_accessible_sponsors(self.user)
        sponsorship_total = sponsors.aggregate(total=Sum('total_contributions'))['total'] or Decimal('0')
        ...
        # Fundraising
        campaigns = get_accessible_fundraising_campaigns(self.user)
        fundraising_total = campaigns.aggregate(total=Sum('amount_raised'))['total'] or Decimal('0')
        ...
        other_income = income_transactions.aggregate(total=Sum('amount'))['total'] or Decimal('0')
        if other_income > 0:
            sources.append({
                'type': 'other_income',
                'name': 'Other Income',
