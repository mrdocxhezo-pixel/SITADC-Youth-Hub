...
class FundraisingProvider:
    ...
    def get_fundraising_summary(self) -> Dict[str, Any]:
        ...
        # Campaigns by status
        by_status = campaigns.values('status').annotate(
            count=Count('id'),
        ...
        recent_launches = campaigns.filter(
            status__in=['launched', 'active']
        ).order_by('-start_date')[:5]  # Assuming start_date field exists
        
        # Campaigns nearing deadline
        # Assuming end_date field exists
        ...
        top_performing = campaigns.annotate(
            progress_percentage= (F('amount_raised') * 100.0) / F('target_amount')
        ).filter(
            target_amount__gt=0
        ...
        return {
            'summary': {
                ...
                'overall_progress_percentage': (total_raised / total_pledged * 100) if total_pledged > 0 else Decimal('0'),
            },
            'by_status': [
                {
                    'status': item['status'],
                    ...
                    'progress_percentage': (item['total_raised'] / item['total_pledged'] * 100) if item['total_pledged'] > 0 else Decimal('0'),
                }
            ...
            'recent_launches': [
                {
                    ...
                    'progress_percentage': (campaign.amount_raised / campaign.target_amount * 100) if campaign.target_amount > 0 else Decimal('0'),
                }
            ...
            'nearing_deadline': [
                {
                    ...
                    'amount_raised': campaign.amount_raised,
                    'end_date': campaign.end_date if hasattr(campaign, 'end_date') else None,
                    'days_remaining': (campaign.end_date - timezone.now()).days if hasattr(campaign, 'end_date') else None,
                    'progress_percentage': (campaign.amount_raised / campaign.target_amount * 100) if campaign.target_amount > 0 else Decimal('0'),
                }
    ...
    def get_fundraising_trends(
        ...
        for campaign in campaigns:
            year = campaign.start_date.year if hasattr(campaign, 'start_date') else campaign.created_at.year
            ...
            'data': [
                {
                    ...
                    'average_target': data['total_pledged'] / data['count'] if data['count'] > 0 else Decimal('0'),
                    'average_raised': data['total_raised'] / data['count'] if data['count'] > 0 else Decimal('0'),
                    'progress_percentage': (data['total_raised'] / data['total_pledged'] * 100) if data['total_pledged'] > 0 else Decimal('0'),
                }
                for year, data in sorted_data
            ],
            'summary': {
                'total_campaigns': sum(data['count'] for data in yearly_data.values()),
                'total_pledged': sum(data['total_pledged'] for data in yearly_data.values()),
                'total_raised': sum(data['total_raised'] for data in yearly_data.values()),
                'total_remaining': sum(data['total_pledged'] - data['total_raised'] for data in yearly_data.values()),
            },
        }
    ...
    def get_fundraising_performance_analysis(
        ...
        Args:
            campaign_id: ID of specific campaign (optional). If None, returns analysis for all campaigns.
        ...
        if campaign_id:
            ...
            try:
                ...
                progress_percentage = (campaign.amount_raised / campaign.target_amount * 100) if campaign.target_amount > 0 else Decimal('0')
                days_elapsed = (timezone.now() - campaign.start_date).days if hasattr(campaign, 'start_date') and campaign.start_date else 0
                days_total = (campaign.end_date - campaign.start_date).days if hasattr(campaign, 'end_date') and hasattr(campaign, 'start_date') and campaign.end_date and campaign.start_date else 0
                days_remaining = (campaign.end_date - timezone.now()).days if hasattr(campaign, 'end_date') else None
                
                # Calculate daily fundraising rate
                daily_rate = (campaign.amount_raised / days_elapsed) if days_elapsed > 0 else Decimal('0')
                
                # Calculate required daily rate to reach goal
                required_daily_rate = (
                    (campaign.target_amount - campaign.amount_raised) / days_remaining 
                    if days_remaining and days_remaining > 0 else Decimal('0')
                )
                ...
                return {
                    ...
                    'performance': {
                        ...
                        'on_track': daily_rate >= required_daily_rate if days_remaining and days_remaining > 0 else None,
                    },
                    'historical_data': []  # Would include historical performance data if available
                }
            except FundraisingCampaign.DoesNotExist:
                raise ValueError(f"Fundraising campaign with ID {campaign_id} does not exist.")
        else:
            ...
            for range_label, min_pct, max_pct in performance_ranges:
                if max_pct is None:
                    ...
                    count = campaigns.filter(
                        amount_raised__gt=F('target_amount')
                    ).count() if hasattr(FundraisingCampaign, 'amount_raised') and hasattr(FundraisingCampaign, 'target_amount') else 0
                else:
                    count = campaigns.filter(
            ...
            campaigns_with_dates = campaigns.filter(
                start_date__isnull=False,
                end_date__isnull=False
            ) if hasattr(FundraisingCampaign, 'start_date') and hasattr(FundraisingCampaign, 'end_date') else campaigns
