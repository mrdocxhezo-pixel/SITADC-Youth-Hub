...
def get_financial_year_for_date(date: timezone.datetime) -> Any | None:
    """
    ...
    try:
        return FinancialYear.objects.get(
            start_date__lte=date, end_date__gte=date, is_active=True
        )
...
def parse_currency_string(currency_string: str) -> tuple[Decimal, str]:
    ...
    Args:
        currency_string: The currency string to parse (e.g., "$1,234.56" or
            "USD 1,234.56").
    ...
    try:
        ...
        raise ValidationError(f"Invalid currency amount: {currency_string}") from exc
    return amount, currency_code


def generate_financial_reference(prefix: str, year: int | None = None) -> str:
    """
    ...
    latest_reference = (
        ReferenceEntry.objects.filter(reference__startswith=f"{prefix}/{year}/")
        .order_by("-reference")
        .first()
    )
    ...
    if latest_reference:
        ...
        try:
            ...
            sequence = int(parts[2]) + 1 if len(parts) >= 3 else 1
...
def validate_financial_amount(amount: Any) -> Decimal:
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid financial amount: {amount}. Error: {e!s}"
        ) from e
...
def get_supported_currencies() -> list[str]:
    ...
    """
    return getattr(
        settings, "SUPPORTED_CURRENCIES", ["USD", "EUR", "GBP", "UGX", "KES", "TZS"]
    )
...
def validate_account_code(account_code: str) -> bool:
    ...
    return not (len(clean_code) < 3 or len(clean_code) > 8)


def get_account_type_from_code(account_code: str) -> str | None:
    """
    ...
    Returns:
        str: The account type (asset, liability, equity, income, expense), or
            None if invalid.
    """
