...
def financial_account_list(request):
    return object_list(request, FinancialAccount, 'finance/financial_account_list.html', 'financial_accounts')
...
def bank_account_list(request):
    return object_list(request, BankAccount, 'finance/bank_account_list.html', 'bank_accounts')
...
def petty_cash_list(request):
    return object_list(request, PettyCash, 'finance/petty_cash_list.html', 'petty_cashes')
...
def fundraising_campaign_list(request):
    return object_list(request, FundraisingCampaign, 'finance/fundraising_campaign_list.html', 'fundraising_campaigns')
...
def procurement_financial_tracking_list(request):
    return object_list(request, ProcurementFinancialTracking, 'finance/procurement_financial_tracking_list.html', 'procurement_financial_trackings')
...
def asset_financial_tracking_list(request):
    return object_list(request, AssetFinancialTracking, 'finance/asset_financial_tracking_list.html', 'asset_financial_trackings')
...
def financial_forecast_list(request):
    return object_list(request, FinancialForecast, 'finance/financial_forecast_list.html', 'financial_forecasts')
...
def budget_allocation_detail(request, pk):
    return object_detail(
        ...
        BudgetAllocation,
        pk,
        'finance:b budget_allocation_list',
...
def reports_balance_sheet(request):
    ...
    if date_str:
        as_of_date = timezone.datetime.strptime(date_str, '%Y-%m-%d').replace(tzinfo=timezone.utc)
    else:
...
def reports_grant_summary(request):
    ...
    if grant_id:
        ...
        data['title'] = f'Grant Report: {data.get("grant", {}).get("name", "Unknown Grant")}'
    else:
...
def analytics_expense_trends(request):
    ...
    data = provider.get_expense_trends(months=months, group_by=group_by, by_source=by_source)
...
def budgeting_variance_report(request):
    ...
    if budget_id:
        ...
        data['title'] = f'Budget Variance Report: {data.get("budget", {}).get("name", "Unknown Budget")}'
    else:
...
def transactions_income(request):
    ...
    data = provider.get_income_transactions(start_date=start_date, end_date=end_date, limit=limit)
...
def transactions_expense(request):
    ...
    data = provider.get_expense_transactions(start_date=start_date, end_date=end_date, limit=limit)
...
def fundraising_performance(request):
    ...
    if campaign_id:
        ...
        data['title'] = f'Fundraising Performance: {data.get("campaign", {}).get("name", "Unknown Campaign")}'
    else:
