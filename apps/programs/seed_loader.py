"""Idempotent loader for program configuration and numbering schemes."""
...
from .models import ProgramReferenceData
from .seed_data import DEFAULT_REFERENCE_DATA, DEFAULT_REFERENCE_SCHEMES


@transaction.atomic
def seed_program_reference_data() -> dict[str, int]:
    ...
    for row in DEFAULT_REFERENCE_DATA:
        _, created = ProgramReferenceData.objects.update_or_create(
            kind=row["kind"],
            code=row["code"],
            defaults={
                "name": row["name"],
