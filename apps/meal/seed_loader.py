"""Idempotent loader for MEAL configuration and numbering schemes."""
...
from .models import MEALReferenceData
from .seed_data import DEFAULT_REFERENCE_DATA, DEFAULT_REFERENCE_SCHEMES


@transaction.atomic
def seed_meal_reference_data() -> dict[str, int]:
    ...
    for row in DEFAULT_REFERENCE_DATA:
        _, created = MEALReferenceData.objects.update_or_create(
            kind=row["kind"],
            code=row["code"],
            defaults={
                "name": row["name"],
